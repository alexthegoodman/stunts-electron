...
export default function VideoPreview({ project = null }: { project: PublicProjectInfo | null }) {
  ...
  useDevEffectOnce(async () => {
    ...
    let isVertical = project.video_data.settings?.dimensions.width === 900 ? false : true
    ...
    const previewUrl = await previewManagerRef.current.generatePreview(sequenceId)
