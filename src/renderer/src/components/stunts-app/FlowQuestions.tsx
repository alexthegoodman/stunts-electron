...
export default function FlowQuestions({
  ...
  const generateHandler = async () => {
    ...
    if (videoContent) {
      ...
      await editor.add_text_item(text_config, new_text, new_id, currentSequenceId)
      ...
      for (const file of flow.flow.content.files) {
        ...
        if (file.mimeType && file.mimeType.includes('video/')) {
          ...
          let video_config = {
            id: new_id,
            name: 'New Video Item',
            dimensions: [file.dimensions.width, file.dimensions.height] as [number, number], // Default video dimensions
            position,
            path: file.url,
            mousePath: '',
            layer: 2
          ...
          let setWidth = file.dimensions.width > 400 ? 400 : file.dimensions.width
          ...
          await editor.add_image_item(image_config, file.url, new Blob(), new_id, currentSequenceId)
    ...
    await saveSequencesData(editorState.savedState.sequences, SaveTarget.Videos)
                ...
                <div className="mb-4">
                  ...
                  <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                    {question?.possibleAnswers?.map((possibleAnswer, answerIndex) => {
                      const isSelected =
                        answersProvided[questionIndex] === possibleAnswer?.answerText

                      return (
                        <button
                          key={answerIndex}
                          onClick={() => {
                            handleAnswerSelection(questionIndex, possibleAnswer?.answerText || '')
                          }}
                          className={`
